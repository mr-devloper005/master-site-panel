import type { SiteTask } from "./site-contract";
...
const taskPayloads: Record<SiteTask, Record<string, unknown>> = {
  listing: {
    ...baseExample,
    content: {
      type: "listing",
...
export const buildTaskProvisioningGuide = (
  task: SiteTask,
  siteCode: string,
...
export const buildTaskCatalog = (siteCode: string, backendBaseUrl?: string | null) => ({
  availableTasks: (Object.keys(taskPayloads) as SiteTask[]).map((task) =>
    buildTaskProvisioningGuide(task, siteCode, backendBaseUrl)
  ),
});
